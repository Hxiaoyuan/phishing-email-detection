...
raw email dataset into more usable formats.
"""
import os
import random
import mailbox
...
def check_text_types(message):
    """
    Check if a message contains text dataset.
...
def mbox_to_df(filename, filepath, text_only=True):
    ...
    check_text_types : Check if a message contains text dataset.
    """
    ...
    if (skip_counter > 0):
        print(skip_counter, "emails skipped: Headers contain non-ascii "\
                "characters, or otherwise corrupted email dataset.")
...
def save_to_csv(data, path, filename):
    ...
    if os.path.exists(attempted_filename):
        ...
        if (overwrite == 'Y' or overwrite == 'y'):
            ...
            print("Aborting, dataset will not be written.")
    else:
